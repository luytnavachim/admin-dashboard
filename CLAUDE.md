...
- **Uren → Moneybird-tijdregistratie**: elke uren-boeking (`addEntry`) maakt óók een time entry in Moneybird (`mbCreateTimeEntry`): gekoppeld aan het contact + het juiste Moneybird-project (gematcht op projectnummer-prefix én budget==uurtarief via `mbFindProjectId`), start 09:00 + duur = aantal uren, `billable: true`. De Moneybird-id wordt op de boeking bewaard (`entry.mbTimeEntryId`). Knop **"Bestaande uren → Moneybird"** (`backfillTimeEntries`) zet bestaande boekingen zonder id alsnog over (idempotent). Elke uren-boeking maakt óók een **Outlook-agenda-afspraak** (`createAgendaEvent` → Graph `POST /users/{AGENDA_CALENDAR}/events`, of `/me/events` als het de ingelogde account is; vereist `Calendars.ReadWrite(.Shared)`). `AGENDA_CALENDAR` staat bovenaan het script (nu `micha@vantuyl.it`); leeg = uit. Werkt rechtstreeks omdat het een eigen mailbox is (`/me` als ingelogd met dat account, anders `/users/{email}` + Calendars.ReadWrite.Shared). Een persoonlijk @outlook.com-account van een ander geeft 404. De event-id wordt bewaard op `entry.outlookEventId`.
