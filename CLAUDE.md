...
- **Moneybird-tab (mailbox ↔ Moneybird controle)**: `reconcileMailboxes()` scant de mailboxen (Graph `searchMessages`, instelbare periode) op inkoopfactuur-mails met PDF, extraheert (UBL/`/extract`), en matcht op factuurnummer tegen Moneybird-inkoopfacturen (`recLoadMbPurchaseMap`). Per mail: **book** (ontbreekt → `mbBookPurchaseFromExtracted`), **attach** (bestaat zonder bijlage → `mbAttachPdfToPurchase`), of **compleet**. Reviewbaar: per regel of bulk (`recBookAll`/`recAttachAll`), dedupe op factuurnummer. Daarnaast op de Facturen-tab een losse "Ontbrekende bijlagen aanvullen" (handmatige PDF-upload per factuur, `loadMissingAttachments`/`mbAttachFileToPurchase`).
